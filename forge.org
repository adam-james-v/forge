...
{:deps 
 {org.clojure/clojure            {:mvn/version "1.10.3"}
  ...
  svg-clj/svg-clj                {:local/root "/home/pi/dev/svg-clj"}}}
   ...
   (let [{:keys [pts quads]} (bmesh/surface surface useg vseg)
         ...
         f (fn [face]
             (let [pts (map #(get pts %) face)]
               (when (= 4 (count (remove nil? pts)))
                 (-> (map drop-last pts)
                     el/polygon
                     (tf/style {:fill "none"
                                :stroke "black"})))))]
     (el/g (remove nil? (map f quads))))))
...
(defn obj
  ([surface seg] (obj surface seg seg))
  ([surface useg vseg]
   (let [surfaces (if (fn? surface) [surface] surface)
         meshes (map #(bmesh/surface % useg vseg) surfaces)
         pts (mapcat :pts meshes)
         quads (mapcat #(apply shift-face-indices %) (map (juxt #(count (:quads %)) #(:quads %)) meshes))
         entryfn (fn [prefix data] (str/join " " (concat [prefix] data)))
         verts-str (str/join "\n" (map #(entryfn "v" %) pts))
...
(def slug
  (let [p (bc/bezier [[0 0 600]
        ...
        bump-t 0.55
        sfn (fn [[ta tb]]
              (let [rfn #(* (svg-clj.utils/ease-in-out-sin ((tri-t bump-t) %)) r)
                    ca (-> (bc/circle (rfn ta))
                           (bc/look-at (p ta) (bc/tangent p ta))
                           (bc/translate (p ta)))
                    cb (-> (bc/circle (rfn tb))
                           (bc/look-at (p tb) (bc/tangent p tb))
                           (bc/translate (p tb)))]
                (bs/surface ca cb)))
        surfaces (map sfn (partition 2 1 (range 0 1.01 0.01)))]
    (mapcat #(drop 2 (svg-surface % 25 1)) surfaces)))
