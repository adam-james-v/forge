...
(def asdf
  (let [a [20 30 40]
    ...
    (mdl/group
     (mdl/polyline [a b])
     (mdl/polyline (map base (range 0 1.05 0.05)))
     (mdl/polyline (map path (range 0 1.05 0.05))))))

(def rev
  (let [xs (bc/bezier [[0 0 0] [100 20 0] [0 100 0]])
        path (-> (bc/circle 20) (bc/translate [50 0 0]))]
    (-> (bs/revolve path [[20 30 40] [70 90 120]])
        (bmesh/surface 20 20)
        mesh->polyhedron
        #_svg-surface2
        #_el/g
        #_(tf/translate [0 0 0])
        #_(tf/style {:fill "white" :stroke "black"}))))
