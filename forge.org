...
{:deps 
 {org.clojure/clojure            {:mvn/version "1.10.2"}
  ...
  scad-clj/scad-clj              {:local/root  "../scad-clj"} #_{:mvn/version "0.5.3"}
...
* freecad-experiments
This some experimentation to see how I can interface forge with FreeCAD to help with file exports so that I can get 'minimum viable product' soon.

Eventually, I want a stable, self-built STEP exporter so that I don't have FreeCAD and OpenCASCADE as heavy dependencies, but this might be ok as a start.

The process pipeline already works like this:

scad-clj -> scad(file) -> csg(file via OpenSCAD) -> step(file via FreeCAD script)

Ideally, this pipeline can work without OpenSCAD,this could be done by modifying scad-clj to output csg directly

** freecad-macro-script
This script makes a cylinder, changes its radius and height, saves a freecad Document and then exports the cylinder to a STEP file.

Idea could be to generate valid macro scripts from my clj library which then can be run with a (sh ...) call.
#+begin_src python
# -*- coding: utf-8 -*-
...
App.newDocument("a")
#App.setActiveDocument("a")

App.ActiveDocument.addObject("Part::Cylinder","c")
App.ActiveDocument.recompute()

FreeCAD.getDocument("a").getObject("c").Radius = '50 mm'
FreeCAD.getDocument("a").getObject("c").Height = '12 mm'
App.getDocument("a").saveAs(u"/Users/adam/dev/forge/out/fcout.FCStd")

__objs__=[]
__objs__.append(FreeCAD.getDocument("a").getObject("c"))
...
** scad-script
Create a scad script and use SCAD to export to .csg.
It would also be possible to directly create a .csg file from clj.

Then, use FreeCAD to import .csg and then export the whole thing as a step file (using similar approach to above).


*** export-script
csg->step via freecad cmd
#+begin_src python :tangle ./fcscript.py
import FreeCAD
import importCSG
import Import
...
App.newDocument(\"a\")
doc = FreeCAD.getDocument(\"a\")
importCSG.insert(u\"/Users/adam/dev/forge/" fin "\", \"a\")
__objs__ = doc.RootObjects
Import.export(__objs__, u\"/Users/adam/dev/forge/" fout "\")
del __objs__"))
...
(defn scad->step
  ...
  (->> scad-block
       (write-csg)
       (spit "out/scadout.csg"))
  (->> f
       (exporter-script "out/scadout.csg")
       (spit "out/fcscript.py"))
  (sh "freecad" "out/fcscript.py")
  (sh "rm" "-rf" 
      "fcscript.py" 
      "parsetab.py"
      "__pycache__")
...
* watcher-proto
#+begin_src clojure :tangle ./w.cljc
(ns forge.watcher
...
(defn -main [args] (design-watcher args))
...
** header
#+begin_src clojure
(defn file-name [author] (str "FILE_NAME('','',('" author "'),(''),'','','');"))
(defn file-description [descr] (str "FILE_DESCRIPTION(('" descr "'),'2;1');"))
(def file-schema "FILE_SCHEMA(('AUTOMOTIVE_DESIGN { 1 0 10303 214 1 1 1 1 }'));")
(def application-protocol-definition "#1 = APPLICATION_PROTOCOL_DEFINITION('international standard','automotive_design',2000,#2);")
(def application-context "#2 = APPLICATION_CONTEXT('core data for automotive mechanical design processes');")
...
(defn footer-b
  ...
  (str (l# ln) " = "
       ...
       (l# (+ ln 4)) ")) GLOBAL_UNIT_ASSIGNED_CONTEXT(("
       (l# (+ ln 1)) ","
       (l# (+ ln 2)) ","
       (l# (+ ln 3)) ")) REPRESENTATION_CONTEXT('Context #1', '3D Context with UNIT and UNCERTAINTY') );"))

(defn footer-units-section
  [ln]
  (str 
   (l# ln) " = " 
   ...
   (l# (+ ln 4)) " = "
   "PRODUCT_RELATED_PRODUCT_CATEGORY('part',$,(#7));\n"))
...
(defn step-circle
  [r]
  (str "#3 = SHAPE_DEFINITION_REPRESENTATION(#4,#10);
#4 = PRODUCT_DEFINITION_SHAPE('','',#5);
#5 = PRODUCT_DEFINITION('design','',#6,#9);
#6 = PRODUCT_DEFINITION_FORMATION('','',#7);
#7 = PRODUCT('wumpus','chumpus','',(#8));
...
#22 = VERTEX_POINT('',#23);
#23 = CARTESIAN_POINT('',(" (float r) ",0.,0.));
#24 = SURFACE_CURVE('',#25,(#30),.PCURVE_S1.);
#25 = CIRCLE('',#26," (float r) ");
...
#33 = CARTESIAN_POINT('',(0.,1.232712559332E-15,0.));
#34 = DIRECTION('',(0.,0.,1.));
#35 = DIRECTION('',(1.,0.,0.));
#36 = DEFINITIONAL_REPRESENTATION('',(#37),#41);
#37 = CIRCLE('',#38," (float r) ");
#38 = AXIS2_PLACEMENT_2D('',#39,#40);
#39 = CARTESIAN_POINT('',(0.,-1.232712559332E-15));
#40 = DIRECTION('',(1.,0.));"))
#+end_src

** linelist
Step files have numbered 'commands'. The numbers look like: #30, #32, etc. and can be thought of like variables, but forward references are allowed.

If I build up a file as a seq. of strings, I can apply numbers at the last step before exporting. This doesn't yet solve the need to fix refs to other line numbers in each line.


Maybe I should have all step export functions work with higher-rder functions.

So, user runs (circle 6)
Which returns a higher-order function expecting the 'seed line number'.

that function will then return a complete list with all numbers filled in properly once the seed is passed in.

#+begin_src clojure
(defn apply-numbers
  [s]
  (for [ln (range (count s))]
    (str (l# (inc ln)) " = " (get s ln))))
...
(defn lame
  [r]
  (str (step-header "adam-james" "first step export... barely counts, but is still something!")
       (step-circle r)
       (step-footer 41)))

#+end_src
** PROTO-STEP-EXPORT
This is a working setup that can export STEP files from SCAD-based .csg files. It uses my local fork of scad-clj to generate .csg files and then run a python script that converts .csg to STEP via the freecad CLI.

This is fine for Proof of Concept, but is a bit clunky and won't work on any web examples without running a server.

*** TODO fix exporter-script to not need hardcoded file paths.

#+begin_src clojure ./src/forge/export.clj
(ns forge.export
  (:require [scad-clj.model :refer :all]
            [scad-clj.csg :refer [write-csg]]
...
App.newDocument(\"a\")
doc = FreeCAD.getDocument(\"a\")
importCSG.insert(u\"/Users/adam/design/capilano/" fin "\", \"a\")
__objs__ = doc.RootObjects
Import.export(__objs__, u\"/Users/adam/design/capilano/" fout "\")
del __objs__"))
