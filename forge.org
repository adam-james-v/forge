...
(defn vint [v] (mapv #(int %) v))

(def pga (mapv #(take 2 %) (regular-polygon-pts 85 6)))
(def pgb (mapv #(v+ [70 30] %) pga))
(def ipts (polygon-intersection pga pgb))

(def la (nth (partition 2 1 (close-path pga)) 5))
(def lb (nth (partition 2 1 (close-path pgb)) 4))
