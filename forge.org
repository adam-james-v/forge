...
{:deps
 {org.clojure/clojure            {:mvn/version "1.10.3"}
  ...
  svg-clj/svg-clj                {:local/root "../svg-clj"}}}
...
* scene
The scene namespace is useful for certain kinds of compilers that may require concepts of cameras and lights. For instance, the SVG compilation inherently involves projecting a 3D object onto a 2D surface, so the question will arise: how is this object being viewed? And the scene will dictate the result.

It might also be very helpful for exporting to blender files or three.js scenes, gltf, that sort of thing.

This namespace will:

- take some mdl
- take a scene map that specifies:
  - a camera map {:type orthographic/perspective :position [x y z] :other necessary options per camera}
  - an optional set of lights [{:position [x y z] :color, etc...}]
  - world details {:up [0 0 1] :axes [1 -1 1] (some way to indicate if a flip must happen)
- from these, calculate the full transform function needed to look through the camera's viewport -> do this as a matrix transform, and return the 4x4 tfrm matrix
- do nothing directly, just return the transform to be used further down the line by a compiler

It'll also be good to provide a helper or two:
- sensible default camera(s) for different applications
- top/right/left/iso camera positions and settings for common CAD views
- some intelligence for 'missing' information -> for example, have 'fit to viewport' kind of stuff so that the user doesn't have to guess camera position and/or zoom levels.
- automatic look-at if camera position is given
...
(defn- svg-single-surface
  ...
  (let [{:keys [pts quads]} (bmesh/surface surface useg vseg)
        pts (mapv drop-last (vec (scene/isometric-xf pts)))
        f (fn [face]
...
* thoughts
I need to focus on getting things to a useful state.
This means making sure I unify BREP/FREP under the mdl namespace, covering all of the OpenSCAD operations
